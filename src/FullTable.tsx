...
export default function FullTable({ onSelect }: Props) {
  return (
    <div className="mx-3 mt-3 mb-6">
      <p className="text-[10px] text-gray-400 mb-2 text-center">A / B 버튼으로 비교 슬롯 선택 · ★ = 전체 최고값</p>
          ...
          <tbody>
            {vitaminData.map((v, idx) => (
              <tr key={v.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-[#F8F9FC]'}>
                <td
                  className={`sticky left-0 z-10 ${idx % 2 === 0 ? 'bg-white' : 'bg-[#F8F9FC]'} px-3 py-2 border-r border-[#E8EAF0]`}
                >
                  <div className="font-semibold text-[#111827] whitespace-nowrap text-[10px]">{v.name}</div>
                  <div className="text-gray-400 text-[8px] mt-0.5">{v.maker}</div>
                  <div className="flex gap-1 mt-1.5">
                    <button
                      onClick={() => onSelect(v.id, 'a')}
                      className="text-[8px] px-1.5 py-0.5 bg-blue-50 text-blue-600 border border-blue-200 rounded hover:bg-blue-100 transition-colors"
                    >A</button>
                    <button
                      onClick={() => onSelect(v.id, 'b')}
                      className="text-[8px] px-1.5 py-0.5 bg-amber-50 text-amber-600 border border-amber-200 rounded hover:bg-amber-100 transition-colors"
                    >B</button>
                  </div>
                </td>
                {cols.map(c => {
